...
**Purpose**: Unofficial TypeScript SDK for Perplexity AI, which is tailored to add the convenient SDK allowing using Perplexity AI in programmatic environments without loosing all the gains you have as Known Service User 
**Sources** 
  - Perplexity AI SPA Vite+Workspace PWA (https://www.perplexity.ai/service-worker.js)
  - HAR files with assets and apis interactions
  - Derivatives that other agents produce (Perplexity AI, CLINE) 
**Protocol**: 2.18  
...
0. **NextAuth Email Flow**:
 - Auth using email with link and otp. OTP is when the AGENT/PROGRAM cant access the email, while link is better for those whose got email to setup cookie
 -   
...
**Request Format**:
```typescript
GET /rest/sse/perplexity_ask?query_str=...&version=2.18&...
Accept: text/event-stream
X-Source-Ref assets/_spa/stream-...:offset
```
...
4. **Add JSDoc** - Document public APIs with examples and references to the exact code from SPA bundle we're reversing 
