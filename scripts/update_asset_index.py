    ...
    for version_dir in sorted(workbox_dir.iterdir()):
        ...
        for file_path in version_dir.rglob("*"):
            if file_path.is_file():
                ...
                files[str(rel_path)] = {"size": file_path.stat().st_size}
...
def update_asset_index(repo_root, verify=False):
    ...
    if verify:
        ...
        if existing.get("total_snapshots") != index["total_snapshots"]:
            print(
                f"Warning: Snapshot count mismatch: {existing.get('total_snapshots')} vs {index['total_snapshots']}"
            )
            ok = False

        if existing.get("total_workbox_versions") != index["total_workbox_versions"]:
            print(
                f"Warning: Workbox version count mismatch: {existing.get('total_workbox_versions')} vs {index['total_workbox_versions']}"
            )
            ok = False

        if existing.get("total_vite_versions") != index["total_vite_versions"]:
            print(
                f"Warning: Vite version count mismatch: {existing.get('total_vite_versions')} vs {index['total_vite_versions']}"
            )
            ok = False

        if existing.get("total_diffs") != index["total_diffs"]:
            print(
                f"Warning: Diff count mismatch: {existing.get('total_diffs')} vs {index['total_diffs']}"
            )
            ok = False
        ...
        if len(existing_assets) != len(index["assets"]):
            print(
                f"Warning: Asset list length mismatch: {len(existing_assets)} vs {len(index['assets'])}"
            )
            ok = False
